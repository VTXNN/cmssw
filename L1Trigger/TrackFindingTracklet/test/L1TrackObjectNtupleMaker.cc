...
private:
  ...
  std::vector<float>* trueMET;
  std::vector<float>* trueMETpx;
  std::vector<float>* trueMETpy;
  std::vector<float>* trueMETphi;

  std::vector<float>* genMET;
  std::vector<float>* genMETpx;
  std::vector<float>* genMETpy;
  std::vector<float>* genMETphi;
...
void L1TrackObjectNtupleMaker::beginJob() {
  ...
  trueMET  = new std::vector<float>;
  trueMETpx = new std::vector<float>;
  trueMETpy = new std::vector<float>;
  trueMETphi = new std::vector<float>;
  genMET = new std::vector<float>;
  genMETpx = new std::vector<float>;
  genMETpy = new std::vector<float>;
  genMETphi = new std::vector<float>;
  ...
  if (SaveTrackJets) {
    ...
    eventTree->Branch("pv_HepMC", &m_pv_HepMC);
  ...
  if (SaveTrackMET) {
    eventTree->Branch("trueMET", &trueMET);
    eventTree->Branch("trueMETPx", &trueMETpx);
    eventTree->Branch("trueMETPy", &trueMETpy);
    eventTree->Branch("trueMETPhi", &trueMETphi);

    eventTree->Branch("genMET", &genMET);
    eventTree->Branch("genMETPx", &genMETpx);
    eventTree->Branch("genMETPy", &genMETpy );
    eventTree->Branch("genMETPhi", &genMETphi );

    if (Displaced=="Prompt" || Displaced=="Both") {
      eventTree->Branch("trkMET", &trkMET);
      eventTree->Branch("trkMHT", &trkMHT);
      eventTree->Branch("trkHT", &trkHT);
    }
    if (Displaced=="Displaced" || Displaced=="Both") {
      eventTree->Branch("trkMETExt", &trkMETExt);
      eventTree->Branch("trkMHTExt", &trkMHTExt);
      eventTree->Branch("trkHTExt", &trkHTExt);
    }
  }
}
  ...
  if (!(MyProcess == 13 || MyProcess == 11 || MyProcess == 211 || MyProcess == 6 || MyProcess == 15 ||
    ...
    if (GenParticleHandle.isValid()) {
      ...
      for (genpartIter = GenParticleHandle->begin(); genpartIter != GenParticleHandle->end(); ++genpartIter) {
        ...
        tempgenMETx += genpartIter -> pt()*cos(genpartIter ->phi());
        tempgenMETy += genpartIter -> pt()*sin(genpartIter ->phi());
      }
      
      m_pv_MC->push_back(zvtx_gen);
      genMET->push_back(sqrt(tempgenMETx*tempgenMETx + tempgenMETy*tempgenMETy));
      genMETpx->push_back(tempgenMETx);
      genMETpy->push_back(tempgenMETy);
      genMETphi->push_back(atan2(tempgenMETy,tempgenMETx));
      
      float temptrueMETx = 0;
      float temptrueMETy = 0;
      ...
      for (size_t i = 0; i<GenParticleHandle->size(); ++i) {
        ...
        if (p.numberOfMothers() != 0) {
          temptrueMETx += p.pt()*cos(p.phi());
          temptrueMETy += p.pt()*sin(p.phi());
        }
      }
      trueMET->push_back(sqrt(temptrueMETx*temptrueMETx + temptrueMETy*temptrueMETy));
      trueMETpx->push_back(temptrueMETx);
      trueMETpy->push_back(temptrueMETy);
      trueMETphi->push_back(atan2(temptrueMETy,temptrueMETx));
    }
    else {
    ...
    for (HepMC::GenEvent::vertex_const_iterator ivertex =
      ...
      std::cout << (*ivertex)->position().x() << "," << (*ivertex)->position().y() << "," << (*ivertex)->position().z() << std::endl;
